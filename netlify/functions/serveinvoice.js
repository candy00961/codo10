// netlify/functions/serveinvoice.js
const fetch = require('node-fetch'); // Make sure 'node-fetch' is in dependencies

exports.handler = async (event) => {
  // NOTE: This function seems identical to fetchKwh.js.
  // Is it intended to do something different, maybe fetch invoice *data*?
  // If it's fetching the *same* data, you might only need one function.
  // Assuming for now it fetches the same read data. Adjust if needed.
  ...
  try {
    console.log(`[serveinvoice] Fetching URL: ${url}`); // Add logging
    const response = await fetch(url, {
      headers: { "Access-Token": apiKey }, // Use the variable
    });
     console.log(`[serveinvoice] Response status: ${response.status}`); // Add logging

    if (!response.ok) {
       // Log error details if fetch failed
       const errorBody = await response.text();
       console.error(`[serveinvoice] API Error ${response.status}: ${errorBody}`);
    ...
    console.error("[serveinvoice] Fetch error:", error); // Log the actual error
